...
#include "qandroidstreamscontrol.h"
#include "qandroidplayersession.h"

QAndroidStreamsControl::QAndroidStreamsControl(QAndroidPlayerSession *session, QObject *parent)
    :QMediaStreamsControl(parent), m_session(session)
{
    connect(m_session, SIGNAL(streamsChanged()), SIGNAL(streamsChanged()));
}

QAndroidStreamsControl::~QAndroidStreamsControl()
{
}

int QAndroidStreamsControl::streamCount()
{
    return m_session->streamCount();
}

QMediaStreamsControl::StreamType QAndroidStreamsControl::streamType(int streamNumber)
{
    return m_session->streamType(streamNumber);
}

QVariant QAndroidStreamsControl::metaData(int streamNumber, QtMultimediaKit::MetaData key)
{
    return m_session->streamProperties(streamNumber).value(key);
}

bool QAndroidStreamsControl::isActive(int streamNumber)
{
    return streamNumber != -1 && streamNumber == m_session->activeStream(streamType(streamNumber));
}

void QAndroidStreamsControl::setActive(int streamNumber, bool state)
{
